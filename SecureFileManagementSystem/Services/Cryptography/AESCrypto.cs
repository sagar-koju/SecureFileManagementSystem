...
namespace SecureFileManagement.Cryptography
{
    public static class AESCrypto
    {
        ...
        // Entry point — Encrypt with AES-256 + HMAC
        public static (byte[] ciphertext, byte[] iv, byte[] hmac) Encrypt(byte[] plaintext, byte[] key, byte[] hmacKey)
        {
            ...
            byte[] hmacInput = iv.Concat(ciphertext).ToArray();
            byte[] tag = hmac.ComputeHash(hmacInput);

            return (ciphertext, iv, tag);
        }

        // Entry point — Decrypt and verify HMAC
        public static byte[] Decrypt(byte[] ciphertext, byte[] key, byte[] iv, byte[] hmac, byte[] hmacKey)
        {
            ValidateKeyLength(key);
            ValidateKeyLength(hmacKey);
            ...
            byte[] expectedHmac = hmacAlg.ComputeHash(iv.Concat(ciphertext).ToArray());
        ...
        // Overloads for string support
        public static (byte[] ciphertext, byte[] iv, byte[] hmac) EncryptString(string plainText, byte[] key, byte[] hmacKey)
        {
            return Encrypt(Encoding.UTF8.GetBytes(plainText), key, hmacKey);
        }

        public static string DecryptToString(byte[] ciphertext, byte[] key, byte[] iv, byte[] hmac, byte[] hmacKey)
        {
            var decryptedBytes = Decrypt(ciphertext, key, iv, hmac, hmacKey);
...
private static byte[] PKCS7Pad(byte[] data)
        {
            ...
            return data.Concat(Enumerable.Repeat((byte)padding, padding)).ToArray();
        }

        private static byte[] PKCS7Unpad(byte[] data)
        {
            int padding = data[^1];
            if (padding < 1 || padding > BlockSize) throw new CryptographicException("Invalid padding.");
            return data.Take(data.Length - padding).ToArray();
        }
        ...
        private static void ValidateKeyLength(byte[] key)
        {
            if (key.Length != KeySize)
                throw new ArgumentException($"Key must be {KeySize} bytes (256 bits) long.");
        }
    }
}
